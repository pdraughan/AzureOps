...
# allows one Secret value to be updated in multiple KeyVaults; assumes same named Secret and multiple keyvaults. if one, remove "[0]" from "$TheSecretName = (Get-AzKeyVaultSecret -VaultName $KeyVaults[0]"
...
$TheSecretName = (Get-AzKeyVaultSecret -VaultName $KeyVaults | Select-Object Name | ogv -Title "Choose ONE Secret name to update" -PassThru).Name
