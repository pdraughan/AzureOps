...
$Path = "c:\scripts\certs.csv"

foreach ($PC in $Computers){


Get-Certificate -Computername $PC.Name |Select Thumbprint, Subject,ExpiresIn, computername |  Export-CSV -Path $Path -UseCulture -Encoding ascii -NoTypeInformation -Append

}


# Get-Certificate -Computername hsterdtsp004  -StoreName "*" | Select Thumbprint, Subject,ExpiresIn, computername